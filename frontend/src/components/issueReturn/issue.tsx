...
function AdminIssueView() {
    ...
    const handleExport = () => {
        ...
        const headers = ["ID,Member Name,Book Title,Issue Date,Due Date,Return Date,Fine,Status"];
        const rows = filtered.map(r => {
            const memberName = (r.member?.full_name || r.member?.username || '').replace(/"/g, '""');
            const bookTitle = (r.book?.title || '').replace(/"/g, '""');
            return `"${r.id}","${memberName}","${bookTitle}","${r.issue_date || ''}","${r.due_date || ''}","${r.return_date || ''}","${r.fine_amount || '0'}","${r.status}"`;
        });
